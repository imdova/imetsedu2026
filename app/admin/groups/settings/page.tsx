...
import {
  ...
  groupSettingsSubCategories,
  type GroupCategoryRecord,
  type GroupSubCategoryRecord,
} from "@/lib/groupSettingsData";
...
function GroupSettingsContent() {
  ...
  const [addedCategories, setAddedCategories] = useState<GroupCategoryRecord[]>([]);
  const [addedSubCategories, setAddedSubCategories] = useState<
    GroupSubCategoryRecord[]
  >([]);

  const [newCatName, setNewCatName] = useState("");
  const [newSubName, setNewSubName] = useState("");
  const [newSubParentId, setNewSubParentId] = useState(
    groupSettingsCategories[0]?.id ?? ""
  );

  const allCategories = useMemo(
    () => [...groupSettingsCategories, ...addedCategories],
    [addedCategories]
  );
  const allSubCategories = useMemo(
    () => [...groupSettingsSubCategories, ...addedSubCategories],
    [addedSubCategories]
  );
              ...
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-gray-200">
                  {filteredCategories.map((c) => (
                    <GroupCategoryRow key={c.id} category={c} />
                  ))}
                </tbody>
              </table>
            </div>
              ...
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-gray-200">
                  {filteredSubCategories.map((s) => (
                    <GroupSubCategoryRow key={s.id} subCategory={s} />
                  ))}
                </tbody>
              </table>
            </div>
...
function GroupCategoryRow({ category }: { category: GroupCategoryRecord }) {
  return (
    <tr className="hover:bg-gray-50/50">
      ...
      <td className="px-4 py-3 text-sm text-gray-900">{category.groups}</td>
      <td className="px-4 py-3">
...
function GroupSubCategoryRow({
  subCategory,
}: {
  subCategory: GroupSubCategoryRecord;
}) {
  return (
    <tr className="hover:bg-gray-50/50">
      <td className="px-4 py-3">
      ...
      <td className="px-4 py-3 text-sm text-gray-900">{subCategory.groups}</td>
      <td className="px-4 py-3">
        <div className="flex items-center gap-1">
          <button
            type="button"
