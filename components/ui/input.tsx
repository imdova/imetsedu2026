...
const inputVariants = cva(
  "flex w-full min-w-0 rounded-md border bg-transparent px-3 py-1  shadow-xs transition-[color,box-shadow] outline-none disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50 text-sm file:inline-flex file:h-7 file:border-0 file:bg-transparent file:text-sm file:font-medium file:text-foreground placeholder:text-muted-foreground selection:bg-primary selection:text-primary-foreground dark:bg-input/30",
  {
    variants: {
      variant: {
        default:
          "h-9 border-border focus-visible:border-primary focus-visible:ring-primary/50 focus-visible:ring-[3px] aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive",
        ghost1:
...
function Input({
  ...
  return (
    <input
      ...
      data-slot="input"
      className={cn(
        inputVariants({ variant, fieldSize, iconPosition }),
        className,
      )}
      {...props}
    />
...
const InputIcon = ({
  ...
  const directionClass = variant === "left" ? "pl-3 left-0" : "pr-3 right-0";
  return (
    <div
      className={cn(
        "text-muted-foreground pointer-events-none absolute flex items-center justify-center text-sm peer-disabled:opacity-50",
        top ? "top-2.5" : "inset-y-0",
        directionClass,
        className,
      )}
    >
...
export { Input, PasswordInput, InputIcon };
